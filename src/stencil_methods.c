...
#include "../include/stencil_utility.h"
...
void applyStateOperatorSubspace_halo(const struct gridContext gc, const int l,
  ...
  const uint_fast32_t ndofc = compute_ndof(&gc,l);
...
void projectToFinerGrid_halo(const struct gridContext gc,
  ...
  uint_fast32_t nelxf,nelyf,nelzf;
  compute_num_cells(&gc,l,&nelxf,&nelyf,&nelzf);

  uint_fast32_t wrapxf,wrapyf,wrapzf;
  compute_wrapping(&gc,l,&wrapxf,&wrapyf,&wrapzf);

  // Coarse grid dimensions
  uint_fast32_t nelxc,nelyc,nelzc;
  compute_num_cells(&gc,l+1,&nelxc,&nelyc,&nelzc);

  uint_fast32_t wrapxc,wrapyc,wrapzc;
  compute_wrapping(&gc,l+1,&wrapxc,&wrapyc,&wrapzc);  
...
#pragma omp parallel for simd safelen(3)
...
#pragma omp parallel for collapse(3) reduction(+ : cc)
  for (int32_t i = 1; i < gc.nelx + 1; i++)
    for (int32_t k = 1; k < gc.nelz + 1; k++)
      for (int32_t j = 1; j < gc.nely + 1; j++) {
        ...
        cblas_dsymv(CblasRowMajor, CblasUpper, 24, 1.0, gc.precomputedKE[0], 24,
                    u_local, 1, 0.0, tmp, 1);
        STYPE clocal = 0.0;
        for (int ii = 0; ii < 24; ii++)
